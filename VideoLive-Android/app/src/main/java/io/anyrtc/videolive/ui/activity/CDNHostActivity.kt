 ...
 * CDN实时直播模式的主播端
 */
class CDNHostActivity : LiveBroadcastBaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val cdnUrl = intent.getStringExtra(Constans.PUSH_URL)
        if (
            rtmToken.isNullOrEmpty() || rtcToken.isNullOrEmpty() ||
            roomId.isNullOrEmpty() || (roomType == 0) || cdnUrl == null
        ) {
            toast("参数异常")
            finish()
            return
        ...
        liveVM.setRoomInfo(roomId, rtmToken, rtcToken, roomType, true, cdnUrl = cdnUrl)
        liveVM.initSDK(this)
        ...
        liveVM.expiredAlert.observe(this) {
            TipDialog.show(this, "体验时间已到期", TipDialog.TYPE.WARNING).setOnDismissListener {
                liveVM.deleteRoom()
                finish()
            }.let {
                it.cancelable = false
